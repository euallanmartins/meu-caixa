...
export function ClientActivityTabs({ cliente }: ActivityTabsProps) {
  ...
  const proximos = sortedAgendamentos.filter(a => new Date(a.data_hora_inicio) >= now);
  const anteriores = sortedAgendamentos.filter(a => new Date(a.data_hora_inicio) < now);
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {displayed.length === 0 ? (
                ...
                displayed.map((app) => {
                  const dateObj = new Date(app.data_hora_inicio);
                  ...
                  return (
                    <div key={app.id} className="group flex items-center justify-between rounded-[2rem] border border-white/5 bg-white/5 p-5 transition-all hover:bg-white/10">
                      <div className="flex min-w-0 items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span className={`rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase tracking-tighter ${getStatusStyle(app.status)}`}>
                              {app.status === 'atendido' ? 'Finalizado' : app.status}
                            </span>
                            <span className="text-[10px] font-black text-white/40">{hora}</span>
                          </div>
                          <h5 className="mt-1 truncate text-xs font-black uppercase tracking-wide text-white">
                            {app.servicos?.nome || 'Servico personalizado'}
                          </h5>
                          <span className="text-[10px] font-bold tracking-tight text-muted opacity-70">
                            {app.barbeiros?.nome || 'Nao atribuido'}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="text-sm font-black text-white">{formatMoney(Number(app.servicos?.valor || app.valor_estimado || 0))}</span>
